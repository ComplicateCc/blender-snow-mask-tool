...
## 测试分支更新：GIM UV V 翻转修正

进一步修正 NeoX GIM 预览导入：

- 坐标仍使用 `X, -Z, Y` 转到 Blender Z-up。
- UV 改为 `(u, 1 - v)`，适配 NeoX/TGA 到 Blender 的 V 方向差异。
- 修复高模 `hp_bank01.gim` 顶部 `BANK` 招牌看起来反向的问题。
- 使用 `model_high_2024/happy101/new/building/hp_bank01.gim` 重新测试，招牌文字方向已正常。
## 测试分支更新：修复正门视角镜像

以 `model_high_2024/happy101/new/building/hp_bank01.gim` 的正门方向验证：

- 正确坐标转换为 `NeoX (X,Y,Z)` -> `Blender (-X,-Z,Y)`。
- 法线使用同样转换。
- 因为坐标转换改变 handedness，GIM 三角面绕序反转为 `(0,2,1)`。
- UV 继续使用 `(u, 1-v)`。
- 正门视角下屋顶 `BANK` 文字已验证为正向。
## 测试分支更新：ParamMap 通道修正

修正 GIM/NeoX ParamMap 在覆雪混合材质中的通道连接：

- `ParamMap.R` -> Roughness
- `ParamMap.G` -> Metallic

此前基础 NeoX 材质已按这个规则连接，但转换成覆雪混合材质时通道被接反。现在 `Mix Roughness With Snow` 使用 Red，`Mix Metallic With Snow` 使用 Green。